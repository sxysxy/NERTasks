...
bert = None
save_model = False
use_pretrained_transition = True
...
for i in range(n_group):
    args = argv[i * 3 : (i+1) * 3]
    cmd = f"python main.py --dataset {args[0]} --model_name {args[1]} --ner_lr {args[2]} --label_smooth_factor {label_smooth_factor} --ner_epoches {ner_epoches} --batch_size {batch_size} --grad_acc {grad_acc} --lstm_layers {lstm_layers} --lstm_hidden_size {lstm_hidden_size} --few_shot {few_shot} {('--bert_name_or_path %s' % bert) if bert else ''} --few_shot_seed {few_shot_seed} {'--save_model' if save_model else ''} {'--use_pretrained_transition' if use_pretrained_transition else ''}"
    os.system(cmd)    
